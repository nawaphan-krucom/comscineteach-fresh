...
function getUnitScoreForStudent(studentId, unit, allProgress) {
...
async function exportModalDetailsToXlsx(student, unitSummary, allProgress) {
  ...
  for (const a2 of unitSummary.assessments) {
    const { score } = getUnitScoreForStudent(student.id, { ...unitSummary, assessments: [a2], totalMaxScore: a2.maxScore || 0 }, allProgress || {});
    const max = a2.maxScore || 0;
    const pct = max ? Math.round(score / max * 100) : 0;
    sheetData.push([a2.title || a2.id, score, max, pct]);
  }
...
async function exportGradebookToXlsx(students, unitsSummary, allProgress) {
  ...
  const ws = utils.aoa_to_sheet(sheetData);
  const wb = utils.book_new();
  utils.book_append_sheet(wb, ws, "Gradebook");
  const wbout = writeSync(wb, { bookType: "xlsx", type: "array" });
  const blob = new Blob([wbout], { type: "application/octet-stream" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = `gradebook_export_${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.xlsx`;
  document.body.appendChild(a);
  a.click();
  a.remove();
  URL.revokeObjectURL(url);
