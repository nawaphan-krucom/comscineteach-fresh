...
export const DataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
      ...
      if (firebaseUser) {
        try {
          ...
          if (userDoc.exists) {
            ...
            const teacherEmail = (metaEnv && (metaEnv.VITE_TEACHER_EMAIL as string)) || 'teacher@sappha.ac.th';
            const isTeacher =
  ...
  const login = useCallback(async (u: User) => {
    ...
    if (u.username === 'teacher') {
        ...
        if (isOnline && auth) {
            try {
                const email = u.username.includes('@') ? u.username : `${u.username}@sappha.ac.th`;
  ...
  const register = useCallback(async (newUser: User) => {
    ...
    try {
        const email = `${newUser.username}@sappha.ac.th`;
  ...
  const sendPasswordResetEmail = useCallback(async (raw: string) => {
    ...
    const normalized = cleaned.includes('@') ? cleaned.replace(/\s+/g, '') : `${cleaned.replace(/\s+/g, '')}@sappha.ac.th`;
